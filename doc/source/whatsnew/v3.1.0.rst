...
Reshaping
^^^^^^^^^
- Bug in :meth:`DataFrame.melt` where a ``var_name`` colliding with an ``id_vars`` label or ``value_name`` silently corrupted output instead of raising :class:`ValueError` (:issue:`65654`)
